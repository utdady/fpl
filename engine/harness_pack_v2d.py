"""E022 evaluation: packaged decision U vs raw fixtures_v2d under v2am_s + rates_v1.

Usage:
    python -m engine.harness_pack_v2d
    python -m engine.harness_pack_v2d --season 2025-26

Both arms: minutes=v2am_s, rates=v1, fixtures=v2d, seed=7.
Control: ILP on raw next_utility(mu_v2d).
Treatment: ILP on next_utility(U) with U=(1-q)*mu_v1 + q*mu_v2d,
           q=clip(recent4/90); as_of_gw<=4 -> q=1.
MAE_60+ scored on mu_v2d for both arms (expect identity).
Production defaults unchanged until explicit promote.
"""
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E022 packaging (control = raw v2d; treat = packaged U) ===")
    ...
    for gw in range(1, 39):
        ...
        # Same seed both fixture versions so mu pairs are aligned.
        v2d = project_all(
        ...
        packaged = apply_packaged_next_utility(
            v2d, v1, recent_minutes=recent, as_of_gw=gw, strategy=strategy,
        )

        try:
            sol_c = solve_squad(snap, v2d, strategy=strategy, objective="next")
        ...
        # MAE on mu_v2d for both arms (identity expected).
        by_v2d = {p.player.id: p for p in v2d}
        for pid, proj in by_v2d.items():
            a = act.get(pid)
            if not a:
                continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E022: packaged U vs raw v2d under v2am_s + rates=v1.")
    ...
    print("[e022] Both arms: minutes=v2am_s + rates=v1 + fixtures=v2d")
    print("[e022] Control = raw next_utility(mu_v2d)")
    print("[e022] Treatment = U=(1-q)*mu_v1+q*mu_v2d; q=clip(recent4/90); gw<=4 -> q=1")
    print("[e022] MAE on mu_v2d both arms. Seed fixed. No production promote.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "pack_v2d_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
