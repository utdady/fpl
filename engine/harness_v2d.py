"""E021 evaluation: fixtures_v2d vs hand fixtures under v2am_s + rates_v1.

Usage:
    python -m engine.harness_v2d
    python -m engine.harness_v2d --season 2025-26

Both arms: minutes_version=v2am_s, rates_version=v1, seed=7.
Control fixtures_version=v1; treatment fixtures_version=v2d.
Production fixtures default remains v1 until an explicit promote.
"""
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E021 fixtures_v2d (control = v2am_s + rates_v1 + fixtures_v1) ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E021: fixtures_v2d vs v1 under v2am_s + rates=v1.")
    ...
    print("[e021] Control = minutes=v2am_s + rates=v1 + fixtures=v1")
    print("[e021] Treatment = minutes=v2am_s + rates=v1 + fixtures=v2d")
    print("[e021] Seed fixed. Home/away 1.10/0.88 frozen. Production fixtures default = v1.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2d_fixtures_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
