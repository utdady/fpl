"""E052-A Phase-2: fixtures=v1_sxg vs fixtures=v1 (decision gate).

Usage:
    python -m engine.harness_v1_sxg_phase2
    python -m engine.harness_v1_sxg_phase2 --season 2024-25
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E052-A Phase-2 v1_sxg (control=fixtures=v1) gate={gate} ===")
...
def write_verdict(results: list[dict], path: Path) -> None:
    ...
    lines = [
        "E052-A Phase-2 verdict: fixtures=v1_sxg vs v1",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E052-A Phase-2: fixtures=v1_sxg vs v1.")
    ...
    print("[e052p2] Control = minutes=v2am_fpla + rates=v1 + fixtures=v1")
    print("[e052p2] Treatment = fixtures=v1_sxg + dated fplcache strength hydrate")
    print("[e052p2] Phase-2 decision gate only (XI0 / FAIL Cap / AGG Cap / g_treat)")
    print("[e052p2] Production stays fixtures=v1")
    results = [eval_season(s, args.strategy) for s in seasons]
    if args.season:
        write_summary(results, OUT_DIR / f"v1_sxg_phase2_{args.season}.csv")
        verdict = "single-season"
    else:
        write_summary(results, OUT_DIR / "v1_sxg_phase2_summary.csv")
        write_verdict(results, OUT_DIR / "e052_phase2_verdict.txt")
        verdict = gate_verdict(results)
    print("\n=== GATE SUMMARY ===")
    for r in results:
        print(
