"""E018 evaluation: v2am_s + rates_v2b_e vs v2am_s + rates_v1.

Usage:
    python -m engine.harness_v2b_e
    python -m engine.harness_v2b_e --season 2025-26

Both arms: minutes_version=v2am_s, seed=7.
Control rates_version=v1; treatment rates_version=v2b_e (form-eligible club prior).
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E018 rates_v2b_e (control = v2am_s + rates_v1) ===")
    ...
    print(
        ...
        f"entered_blank={entered_blank_pct:.1f}% mean_entered_dmu={mean_dmu:.3f}"
    )
    return r
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E018: rates_v2b_e vs rates_v1 under frozen v2am_s.")
    ...
    print("[e018] Control = minutes=v2am_s + rates=v1")
    print("[e018] Treatment = minutes=v2am_s + rates=v2b_e (form-eligible club prior)")
    print("[e018] Seed fixed. No alpha/threshold search. Production rates default = v1.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2b_e_rates_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
