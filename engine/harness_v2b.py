"""E016 evaluation: v2am_s + rates_v2b vs v2am_s + rates_v1.

Usage:
    python -m engine.harness_v2b
    python -m engine.harness_v2b --season 2025-26

Both arms: minutes_version=v2am_s, seed=7.
Control rates_version=v1; treatment rates_version=v2b.
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E016 rates_v2b (control = v2am_s + rates_v1) ===")
    ...
    c_xi0, c_z, c_n = xi0(season, ctrl_xi)
    t_xi0, t_z, t_n = xi0(season, treat_xi)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E016: rates_v2b vs rates_v1 under frozen v2am_s.")
    ...
    print("[e016] Control = minutes=v2am_s + rates=v1")
    print("[e016] Treatment = minutes=v2am_s + rates=v2b (xg/xa multi-season club prior)")
    print("[e016] Seed fixed. ATK/CONCEDE/dc/saves/bonus locked. Production rates default = v1.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2b_rates_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
        print(
            f"{r['season']:8} "
            ...
            f"[{'PASS' if all(r[k] for k in ('mae60_ok','spearman60_ok','xicap_ok','xi0_ok')) else 'FAIL'}]"
        )
