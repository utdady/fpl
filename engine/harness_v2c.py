"""E019 evaluation: v2c role-transition minutes vs v2am_s control.

Usage:
    python -m engine.harness_v2c
    python -m engine.harness_v2c --season 2025-26

Both arms: rates_version=v1, seed=7.
Control minutes_version=v2am_s; treatment minutes_version=v2c.
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E019 minutes_v2c (control = v2am_s + rates_v1) ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E019: minutes_v2c vs v2am_s under rates=v1.")
    ...
    print("[e019] Control = minutes=v2am_s + rates=v1")
    print("[e019] Treatment = minutes=v2c + rates=v1 (role-transition competition demotion)")
    print("[e019] Seed fixed. No param search. Production minutes default = v2am_s.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2c_minutes_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
