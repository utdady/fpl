"""E044-A evaluation: v2am_fpla vs v2am_s control.

Usage:
    python -m engine.harness_v2am_fpla
    python -m engine.harness_v2am_fpla --season 2024-25

Both arms: rates=v1, fixtures=v1, seed=7.
Control minutes_version=v2am_s; treatment minutes_version=v2am_fpla.
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E044-A v2am_fpla (control=v2am_s) gate={gate} ===")
    ...
    for gw in range(1, 39):
        ...
        treat = project_all(
            snap, horizon=1, strategy=strategy, seed=SEED,
            minutes_version="v2am_fpla", rates_version="v1",
            fpla_diags_out=fpla_diags,
        )
        ...
        for d in fpla_diags:
            ...
            diag_rows.append({
                ...
                "status": d.status,
                "chance_this": "" if d.chance_this is None else d.chance_this,
                "chance_next": "" if d.chance_next is None else d.chance_next,
                "can_select": int(d.can_select),
    ...
    print(
        ...
        f"g_treat mean={g_mean:.3f} | joined={n_joined} avail<1={n_avail_lt1}"
    )
    return r
...
def write_summary(results: list[dict], path: Path) -> None:
    ...
    fields = [
        ...
        "g_treat_mean", "n_joined_player_gw", "n_avail_lt1", "n_gw",
...
def write_diags(results: list[dict], path: Path) -> None:
    ...
    if not rows:
        return
    # Diags are large; keep only rows with availability < 1 or identity
    slim = [
        row for row in rows
        if row.get("availability0", 1) < 1.0 - 1e-12
        or row.get("identity_reason")
        or not row.get("joined")
    ]
    # If almost everyone joined with avail=1, still sample identity/join_miss
    if len(slim) < 50:
        slim = rows
    ...
    with path.open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
        w.writerows(slim if slim else rows)
    print(f"Wrote {path} ({len(slim if slim else rows)} rows; full joined={len(rows)})")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E044-A: v2am_fpla vs v2am_s.")
    ...
    print("[e044] Control = minutes=v2am_s + rates=v1")
    print("[e044] Treatment = minutes=v2am_fpla (fplcache -> availability())")
    print("[e044] Frozen hydrate; no new lambda/caps. Production stays v2am_s.")
    ensure_fplcache_avail(seasons)
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2am_fpla_summary.csv")
    write_diags(results, OUT_DIR / "v2am_fpla_diagnostics.csv")
    ...
    for r in results:
        print(
            ...
            f"joined={r['n_joined_player_gw']} avail<1={r['n_avail_lt1']}"
        )
    print(f"\nVERDICT: {verdict}")
