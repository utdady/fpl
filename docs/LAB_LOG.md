...
- **Scope lock — promote bar; q frozen from E022; rates contrast = E016 max:**
  ...
  MAE_60+: control on μ_v1; treatment on μ_v2b
  seed=7; objective=next; four seasons GW1–38
  ```

- **Named risk (pre-registered):** **XI0** primary; Cap secondary.

- **Method:** `engine/packaging.py` + `python -m engine.harness_pack_rates` (seed=7).

- **Results (vs production rates=v1):**

  | Season | MAE60 | XI+Cap | XI0% | swaps | Gate |
  |---|---|---:|---:|---:|---|
  | 2022/23 | 2.576→2.559 ✓ | 57.1→**54.0** ✗ | 11.5→**11.3** ✓ | 104 | **FAIL** (Cap) |
  | 2023/24 | 2.482→2.474 ✓ | 53.7→**55.6** ✓ | 11.0→**10.5** ✓ | 61 | **PASS** |
  | 2024/25 | 2.400→2.386 ✓ | 56.6→**58.2** ✓ | 10.5→**10.3** ✓ | 68 | **PASS** |
  | 2025/26 | 2.572→2.562 ✓ | 50.4→**50.1** ✗ | 14.1→14.1 ✓ | 61 | **FAIL** (Cap) |

  MAE✓ 4/4. **XI0✓ 4/4** (named risk did *not* fire — packaging helped blanks vs raw E016). Cap✗ on **2022/23 and 2025/26** — same seasons as raw rates Cap failures (E016/E017).
...
- **Status:** **concentrated** — Cap damage is selection / value-when-playing, not blank toxicity
- **Hypothesis:** E024 Cap✗ on 2022/23+2025/26 with XI0✓ suggests packaging fixed blanks but left **wrong-player-when-playing**. If Cap-FAIL entrants play 60+ yet underperform (or eject high scorers), Cap is a separate selection failure — not a reason to enlarge q.
- **Question:** Under E024 control→treatment XI swaps, do Cap-FAIL seasons (2022-23, 2025-26) differ from Cap-PASS (2023-24, 2024-25) in blank%, μΔ, and pts|60+ for entered vs left?

- **Scope lock — diagnostic only; q frozen; no promote card:**
  ```text
  control XI:  rates=v1, ILP on raw μ
  treat XI:    rates=v2b, ILP on packaged U (q frozen E022)
  movers:      entered = treat\control; left = control\treat
  FAIL seasons: 2022-23, 2025-26
  PASS seasons: 2023-24, 2024-25
  ```

- **Method:** `python scripts/e024b_cap_fail_movers.py` (seed=7).

- **Results (entered / left movers):**

  | Cell | n | blank% | mean_μΔ | pts\|60+ (n) |
  |---|---:|---:|---:|---:|
  | FAIL entered | 165 | 10.9 | 0.632 | **4.25** (122) |
  | PASS entered | 129 | 9.3 | 0.506 | **4.86** (98) |
...
### E028 - Local substitution stability
- **Date:** 2026-09-01 (after E027 REJECT)
- **Status:** **reject (local branch)** — bad swaps on FAIL are margin-agnostic; does not support H-PACK2 near-margin gate
...
- **Method:** `python scripts/e028_local_substitution.py` (804 pairs; ε=0.791).

- **Results (ctrl_treat, both≥60 bad_swap%):**

  | Gate | all | near | mid | large |
  |---|---:|---:|---:|---:|
  | FAIL | **45.0** | 44.9 | 44.7 | 46.7 |
  | PASS | **28.0** | 21.2 | 30.3 | 46.7 |

  Central test: on FAIL, P(bad|near) ≈ P(bad|large) — **no concentration in small \(|M_{ij}|\)**. FAIL vs PASS gap exists at all margin levels (overall 45% vs 28%). ctrl_pack1 similar (FAIL near 39.7%, large 66.7% n=6 only). treat_pack1 n small — inconclusive.
