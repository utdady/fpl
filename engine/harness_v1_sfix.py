"""E048-A evaluation: fixtures=v1_sfix vs fixtures=v1 under v2am_fpla + rates=v1.

Usage:
    python -m engine.harness_v1_sfix
    python -m engine.harness_v1_sfix --season 2024-25

Both arms: minutes=v2am_fpla, rates=v1, seed=7.
Control fixtures=v1 (literal _str → modern overall clamps to 5).
Treatment fixtures=v1_sfix (_str_sfix linear 1000..1350 → {2..5}).
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E048-A v1_sfix (control=fixtures=v1) gate={gate} ===")
    ...
    for gw in range(1, 39):
        ...
        treat = project_all(
            snap, horizon=1, strategy=strategy, seed=SEED,
            minutes_version="v2am_fpla", rates_version="v1", fixtures_version="v1_sfix",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E048-A: fixtures=v1_sfix vs v1.")
    ...
    print("[e048] Control = minutes=v2am_fpla + rates=v1 + fixtures=v1 (_str)")
    print("[e048] Treatment = fixtures=v1_sfix (_str_sfix; STR_LO=1000 STR_HI=1350)")
    print("[e048] Same teams; ATK/CONCEDE unchanged. Production stays fixtures=v1.")
    results = [eval_season(s, args.strategy) for s in seasons]
    if args.season:
        write_summary(results, OUT_DIR / f"v1_sfix_summary_{args.season}.csv")
    else:
        write_summary(results, OUT_DIR / "v1_sfix_summary.csv")
