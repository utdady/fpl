"""Merge per-season E048-A summaries and print gate verdict."""
...
for s in ("2022-23", "2023-24", "2024-25", "2025-26"):
    p = OUT / f"v1_sfix_summary_{s}.csv"
    if not p.exists():
...
with (OUT / "v1_sfix_summary.csv").open("w", encoding="utf-8", newline="") as f:
...
print(f"n_mu_delta_total={n_mu} XI0_all={xi0_all} MAE_all={mae_all} "
      f"FAIL_Cap={fail_cap} AGG {agg_c:.2f}->{agg_t:.2f} ok={agg_ok}")
print(f"VERDICT: {verdict}")
(OUT / "e048_v1_sfix_verdict.txt").write_text(
    f"VERDICT: {verdict}\nn_mu_delta={n_mu}\n"
