"""E053-A Phase-1: fixtures=v1_adxg vs fixtures=v1 (projection-only).

Usage:
    python -m engine.harness_v1_adxg
    python -m engine.harness_v1_adxg --season 2024-25
    python -m engine.harness_v1_adxg --smoke-gw 20 --season 2024-25

Both arms: minutes=v2am_fpla, rates=v1, seed=7.
Control fixtures=v1. Treatment fixtures=v1_adxg + dated ATK/DEF overlay.
...
def eval_season(
    ...
    *,
    ...
    print(f"\n=== {season} E053-A Phase-1 v1_adxg (control=fixtures=v1) gate={gate} ===")
...
def write_verdict(results: list[dict], path: Path) -> None:
    ...
    lines = [
        "E053-A Phase-1 verdict: fixtures=v1_adxg vs v1",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E053-A Phase-1: fixtures=v1_adxg vs v1.")
    parser.add_argument("--season", choices=SUPPORTED_SEASONS, default=None)
    parser.add_argument("--smoke-gw", type=int, default=None)
    args = parser.parse_args()

    print("[e053] Control = minutes=v2am_fpla + rates=v1 + fixtures=v1")
    print("[e053] Treatment = fixtures=v1_adxg + dated ATK/DEF overlay")
    print("[e053] Phase-1 projection only — no Cap/XI0/ILP")
    print("[e053] Production stays fixtures=v1")

    seasons = (args.season,) if args.season else SUPPORTED_SEASONS
    results = [eval_season(s, smoke_gw=args.smoke_gw) for s in seasons]
    if args.smoke_gw is not None:
        tag = f"smoke_gw{args.smoke_gw}"
        out = (
            OUT_DIR / f"v1_adxg_{tag}_{args.season}.csv"
            if args.season else OUT_DIR / f"v1_adxg_{tag}.csv"
        )
        write_summary(results, out)
        print(f"[e053] smoke done (GW{args.smoke_gw}); not a Phase-1 verdict")
        return
    if args.season:
        write_summary(results, OUT_DIR / f"v1_adxg_phase1_{args.season}.csv")
        write_verdict(results, OUT_DIR / f"e053_phase1_verdict_{args.season}.txt")
    else:
        write_summary(results, OUT_DIR / "v1_adxg_phase1_summary.csv")
        write_verdict(results, OUT_DIR / "e053_phase1_verdict.txt")
