"""E020 evaluation: v2c_e cold-eligible demotion vs v2am_s control.

Usage:
    python -m engine.harness_v2c_e
    python -m engine.harness_v2c_e --season 2025-26

Both arms: rates_version=v1, seed=7.
Control minutes_version=v2am_s; treatment minutes_version=v2c_e.
Same demotion rungs as v2c; demotion only when recent4 < 90 (as_of_gw > 4).
Production minutes default remains v2am_s until an explicit promote.
"""
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E020 minutes_v2c_e (control = v2am_s + rates_v1) ===")
    ...
    for gw in range(1, 39):
        ...
        treat = project_all(
            snap, horizon=1, strategy=strategy, seed=SEED,
            minutes_version="v2c_e", rates_version="v1",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E020: minutes_v2c_e vs v2am_s under rates=v1.")
    ...
    print("[e020] Control = minutes=v2am_s + rates=v1")
    print("[e020] Treatment = minutes=v2c_e + rates=v1 (demote only if recent4 < 90)")
    print("[e020] Seed fixed. Rungs frozen from E019. Production minutes default = v2am_s.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "v2c_e_minutes_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
