"""E045-A evaluation: rates=v1_ep vs rates=v1 under frozen v2am_fpla.

Usage:
    python -m engine.harness_v1_ep
    python -m engine.harness_v1_ep --season 2024-25

Both arms: minutes=v2am_fpla, fixtures=v1, seed=7.
Control rates=v1; treatment rates=v1_ep (dated fplcache ep_next blend).
Production stays rates=v1 until SURVIVE + promote. No lambda search.
"""
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    for gw in range(1, 39):
        ...
        treat = project_all(
            snap, horizon=1, strategy=strategy, seed=SEED,
            minutes_version="v2am_fpla", rates_version="v1_ep", fixtures_version="v1",
            ep_diags_out=ep_diags,
        )
        ...
        # Keep a compact sample: blended players only, first 40 per GW.
        sample_n = 0
        for d in ep_diags:
            if d.blended:
                n_blended += 1
                if sample_n < 40:
                    ...
                    diag_rows.append({
                        ...
                        "web_name": d.web_name,
                        "mu0": round(d.mu0, 6),
                        "ep_next": "" if d.ep_next is None else round(d.ep_next, 6),
                        "mu1": round(d.mu1, 6),
                        "blended": 1,
                        "identity_reason": "",
                    })
    ...
    print(
        ...
        f"g_treat mean={g_mean:.3f} | blended={n_blended}"
    )
    return r
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E045-A: rates=v1_ep vs rates=v1.")
    ...
    print("[e045] Control = minutes=v2am_fpla + rates=v1 + fixtures=v1")
    print("[e045] Treatment = rates=v1_ep (dated fplcache ep_next, lambda=0.35)")
    print("[e045] Frozen blend; no Vaastav xP. Production stays rates=v1.")
    ensure_fplcache_ep(seasons)
    results = [eval_season(s, args.strategy) for s in seasons]
    if args.season:
        write_summary(results, OUT_DIR / f"v1_ep_summary_{args.season}.csv")
        write_diags(results, OUT_DIR / f"v1_ep_diagnostics_{args.season}.csv")
    else:
        write_summary(results, OUT_DIR / "v1_ep_summary.csv")
        write_diags(results, OUT_DIR / "v1_ep_diagnostics.csv")
    ...
    for r in results:
        print(
            ...
            f"g_treat={r['g_treat_mean']:.3f} blended={r['n_blended']}"
        )
    print(f"\nVERDICT: {verdict}")
