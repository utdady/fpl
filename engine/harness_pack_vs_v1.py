"""E023 evaluation: packaged fixtures_v2d vs production fixtures_v1.

Usage:
    python -m engine.harness_pack_vs_v1
    python -m engine.harness_pack_vs_v1 --season 2025-26

Control:  minutes=v2am_s + rates=v1 + fixtures=v1, ILP on raw mu_v1
Treatment: minutes=v2am_s + rates=v1 + fixtures=v2d, ILP on packaged U
           U=(1-q)*mu_v1+q*mu_v2d; q frozen from E022
MAE_60+: control on mu_v1; treatment on mu_v2d
...
def eval_season(season: str, strategy: str = "balanced") -> dict:
    ...
    print(f"\n=== {season} E023 packaged v2d vs production fixtures=v1 ===")
    ...
    for gw in range(1, 39):
        ...
        for pid, a in act.items():
            ...
            if pid in by_v2d:
                treat_mae_p.append(by_v2d[pid].next_mu)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E023: packaged v2d vs production fixtures=v1.")
    ...
    print("[e023] Control = minutes=v2am_s + rates=v1 + fixtures=v1 (production)")
    print("[e023] Treatment = fixtures=v2d + packaged U (q frozen from E022)")
    print("[e023] Named risk: XI0. Seed fixed. PASS != auto-promote.")
    results = [eval_season(s, args.strategy) for s in seasons]
    write_summary(results, OUT_DIR / "pack_vs_v1_summary.csv")
    print("\n=== GATE SUMMARY ===")
    for r in results:
